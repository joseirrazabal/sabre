...
function FormatDateTime(when) {
	const dateTimeString = `${when.getFullYear()}-${(when.getMonth() + 1).toString(10).padStart(2, '0')}-${when
		.getDate()
		.toString(10)
		.padStart(2, '0')}T${when
		.getHours()
...
class CreatePassengerNameRecordModel {
	...
	async create() {
		var requestObject = {
			service: `/v2.1.0/passenger/records?mode=create`,
...
export default CreatePassengerNameRecordModel
