...
class BargainFinderMaxView {
	...
	async displayItineraries() {
		...
		itineraries.forEach((itinerary, index) => {
			...
			if (lowPrice.LowestFare.Fare > itinerary.LowestFare.Fare) {
				lowPrice = itinerary
			}
        ...
        <div style="margin-bottom: 20px; border: 1px solid red;">
          ...
          <div>
            Destino: ${this.flightData.DestinationLocation}
          </div>
          <div>
          ...
          <div>
            Link: ${itinerary.Links[0].href}
          </div>
			  </div>`
		})

		result += `<div>
      Menor precio: ${lowPrice.LowestFare.Fare}
      Link: ${lowPrice.Links[0].href}
      </div>`
	}
