...
class BargainFinderMaxView {
	...
	displayNumberOfItineraries() {
      ...
      <div class="row" style="margin-bottom: 20px">
		    <div class="col-md-12">
          ${this.flightData.PricedItineraries.length} itineraries found
        </div>
      </div>
      `
	}
	...
	async displayItineraries() {
		...
		const itineraries = this.flightData.PricedItineraries
		itineraries.forEach((itinerary, index) => {
			result += `
        <div style="margin-bottom: 20px; border: 1px solid red;">
          <div>
							${itinerary.AirItineraryPricingInfo.PTC_FareBreakdowns.PTC_FareBreakdown.PassengerFare.Taxes.TotalTax.Amount}
          </div>
          <div>
							${itinerary.AirItineraryPricingInfo.PTC_FareBreakdowns.PTC_FareBreakdown.PassengerFare.TotalFare.Amount}
          </div>
			  `

			itinerary.AirItinerary.OriginDestinationOptions.OriginDestinationOption.forEach(item => {
				result += `
				    <div>
				      ${item.FlightSegment[0].DepartureAirport.LocationCode}
				    </div>
				    <div>
				      ${item.FlightSegment[0].ArrivalAirport.LocationCode}
				    </div>
				    <div>
				      ${item.FlightSegment[0].MarketingAirline.Code}
				    </div>
				    <div>
				      ${item.FlightSegment[0].DepartureDateTime}
				    </div>
				    <div>
				      ${item.FlightSegment[0].ArrivalDateTime}
				    </div>
				    <div>
				      ${item.FlightSegment[0].FlightNumber}
				    </div>
				  `
			})
